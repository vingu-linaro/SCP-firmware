...
struct smt_channel_ctx {
    ...
    /* Driver API */
    struct mod_optee_smt_driver_output_api *driver_api;
...
static int smt_respond(fwk_id_t channel_id, const void *payload, size_t size)
{
    ...
    memory = ((struct mod_optee_smt_memory*)channel_ctx->mailbox_address);
...
static int smt_signal_message(fwk_id_t channel_id, void *memory)
{
    ...
    channel_ctx->mailbox_address = (uintptr_t)memory;

...
struct mhu_smt_channel {
    ...
    fwk_id_t id;
    struct mod_virtio_smt_driver_input_api *api;
};
...
static void vring_put_prev_req(const struct device *dev, struct mhu_smt_channel *smt_channel, size_t size)
{
     ...
     * When we master the bus, we provisionned this buffer for the other side and must
...
const struct mod_virtio_smt_driver_ouput_api vring_mod_smt_driver_api = {
...
int vring_init_channel(struct mhu_device_ctx *device_ctx, unsigned int slot_count)
{
    ...
    for (i = 0; i < slot_count; i++) {
        ...
        /* preallocate buffers : only necessary for mmio master */
        ...
        if (i == VIRTIO_SCMI_VQ_REQ) {
            ...
            /* preallocated buffers : only necessary for mmio master */
            ...
            if (smt_channel->vqbufs[VIRTIO_SCMI_IN_MSG].buf != NULL)
                /*
                 * When master of the link, buffer is not null and it needs to

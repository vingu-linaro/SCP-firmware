 ...
 */
static int vring_get_next_req(const struct device *dev, struct mbx_smt_channel *smt_channel)
{
...
static void vring_put_prev_req(const struct device *dev, struct mbx_smt_channel *smt_channel, size_t size)
{
    struct virtqueue_buf *elem = smt_channel->vqbufs;
...
static int raise_queue_notify(fwk_id_t channel_id, size_t size)
{
    ...
    struct mbx_smt_channel *smt_channel = &device_ctx->smt_channel_table[chnl_idx];
...
{
    struct mbx_device_ctx *device_ctx = user_data;
    struct mbx_smt_channel *smt_channel;
    unsigned int slot;
...
static void vring_virtio_cb(struct virtqueue *vq)
{
    ...
    struct mbx_device_ctx *device_ctx = driver_data->user_data;
    struct mbx_smt_channel *smt_channel = &device_ctx->smt_channel_table[vq->vq_queue_index];
    uint32_t flags;
     ...
     * new request.
     */
    flags = osThreadFlagsSet(mbx_ctx.os_thread_id, SIGNAL_ISR_EVENT);
    if ((int32_t)flags < 0) {
...
int vring_init_channel(struct mbx_device_ctx *device_ctx, unsigned int slot_count)
{
    struct mbx_smt_channel *smt_channel;
                           ...
                           const void *data)
{
    ...
    device_ctx->config = (struct mod_vring_mbx_channel_config*)data;
                      ...
                      const void **api)
{
    ...
    device_ctx = &mbx_ctx.device_ctx_table[elt_idx];
...
int vring_thread_create(struct mbx_ctx *module_ctx)
{
...
{
    vring_thread_create(&mbx_ctx);
    return FWK_SUCCESS;
}

/* VRING_MBX module definition */
const struct fwk_module module_vring_mbx = {

...
static int get_rate(fwk_id_t dev_id, uint64_t *rate)
{
    ...
    *rate = clk_get_rate(ctx->clk);
    ...
    FMSG("SCMI optee_clock (%u/\"%s\"): clk_get_rate() = %"PRIu64,
...
static int set_state(fwk_id_t dev_id, enum mod_clock_state state)
{
    ...
    if (state == MOD_CLOCK_STATE_STOPPED) {
        if (ctx->enabled) {
            FMSG("SCMI optee_clock (%u/\"%s\") disable",
                 fwk_id_get_element_idx(dev_id), clk_get_name(ctx->clk));

            clk_disable(ctx->clk);
            ctx->enabled = false;
        } else {
            FMSG("SCMI optee_clock (%u/\"%s\") is already OFF",
                fwk_id_get_element_idx(dev_id), clk_get_name(ctx->clk));
        }
    } else {
        if (!ctx->enabled) {
            FMSG("SCMI optee_clock (%u/\"%s\") enable",
                 fwk_id_get_element_idx(dev_id), clk_get_name(ctx->clk));

            clk_enable(ctx->clk);
            ctx->enabled = true;
        } else {
            FMSG("SCMI optee_clock (%u/\"%s\") is already ON",
                fwk_id_get_element_idx(dev_id), clk_get_name(ctx->clk));
        }
    }
...
static int get_state(fwk_id_t dev_id, enum mod_clock_state *state)
{
    ...
    FMSG("SCMI optee_clock (%u/\"%s\") is %s",
         fwk_id_get_element_idx(dev_id), clk_get_name(ctx->clk),
                    ...
                    enum mod_clock_round_mode round_mode)
{
    ...
    FMSG("SCMI optee_clock (%u/\"%s\"): rate = %"PRIu64,
         fwk_id_get_element_idx(dev_id), clk_get_name(ctx->clk), rate);
                               ...
                               unsigned int rate_index, uint64_t *rate)
{
    ...
    assert(!res && rate_count == 1);
    *rate = rate_ul;

    FMSG("SCMI optee_clock (%u/\"%s\"): rate(index %u) = %lu",

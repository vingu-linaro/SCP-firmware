...
struct smt_channel_ctx {
    ...
    struct mod_virtio_smt_channel_config *config;

    /* Channel read and write cache memory areas */
    struct mod_virtio_smt_memory *in;
    struct mod_virtio_smt_memory *out;
    size_t in_len;
    size_t out_len;
...
static int smt_get_secure(fwk_id_t channel_id, bool *secure)
{
    ...
    /* No virtio mailbox is considered secure */
    *secure = 0;
...
{
    struct smt_channel_ctx *channel_ctx;
    struct mod_virtio_smt_memory *memory;

    channel_ctx = &smt_ctx.channel_ctx_table[fwk_id_get_element_idx(channel_id)];
    memory = ((struct mod_virtio_smt_memory *) channel_ctx->out);
...
int virtio_signal_message(fwk_id_t channel_id, void *msg_in, size_t in_len, void *msg_out, size_t out_len)
{
    struct smt_channel_ctx *channel_ctx;
    ...
    if (!channel_ctx->virtio_smt_mailbox_ready) {
        /* Discard any message in the mailbox when not ready */
        FWK_LOG_ERR("[VIRTIO_SMT] Message not valid\n");
    ...
    channel_ctx->in = (struct mod_virtio_smt_memory *)msg_in ;
    channel_ctx->in_len = in_len;
    channel_ctx->out = (struct mod_virtio_smt_memory *)msg_out;
    channel_ctx->out_len = out_len;

    switch (channel_ctx->config->type) {
    case MOD_VIRTIO_SMT_CHANNEL_TYPE_MASTER:
        return smt_master_handler(channel_ctx);
    case MOD_VIRTIO_SMT_CHANNEL_TYPE_SLAVE:
...
static const struct mod_virtio_smt_driver_input_api driver_input_api = {
    .signal_message = virtio_signal_message,
};

/*
 * Framework API
 */
static int virtio_init(fwk_id_t module_id, unsigned int element_count,
                       const void *data)
{
...
static int virtio_channel_init(fwk_id_t channel_id, unsigned int slot_count,
                            const void *data)
{
    size_t elt_idx = fwk_id_get_element_idx(channel_id);
    struct smt_channel_ctx *channel_ctx = &smt_ctx.channel_ctx_table[elt_idx];

    channel_ctx->config = (struct mod_virtio_smt_channel_config*)data;

    /* Validate channel config */
    if (channel_ctx->config->type >= MOD_VIRTIO_SMT_CHANNEL_TYPE_COUNT) {
        assert(false);
        return FWK_E_DATA;
    }
    ...
    channel_ctx->max_payload_size = channel_ctx->config->mailbox_size -
        sizeof(struct mod_virtio_smt_memory);

    channel_ctx->virtio_smt_mailbox_ready = true;

    return FWK_SUCCESS;
}

static int virtio_bind(fwk_id_t id, unsigned int round)
{
    int status;
    struct smt_channel_ctx *channel_ctx;
...
static int virtio_process_bind_request(fwk_id_t source_id,
                                       fwk_id_t target_id,
                                       fwk_id_t api_id,
                                       const void **api)
{
    ...
    case MOD_VIRTIO_SMT_API_IDX_DRIVER_INPUT:
        /* Driver input API */
    ...
    case MOD_VIRTIO_SMT_API_IDX_SCMI_TRANSPORT:
        /* SCMI transport API */
...
static int virtio_start(fwk_id_t id)
{
    return FWK_SUCCESS;
}

const struct fwk_module module_virtio_smt = {
    .type = FWK_MODULE_TYPE_SERVICE,
    .api_count = MOD_VIRTIO_SMT_API_IDX_COUNT,
    .init = virtio_init,
    .element_init = virtio_channel_init,
    .bind = virtio_bind,
    .start = virtio_start,
    .process_bind_request = virtio_process_bind_request,
};

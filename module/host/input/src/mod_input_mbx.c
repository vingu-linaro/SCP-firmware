...
struct mbx_device_ctx {
    ...
    struct mod_input_mbx_channel_config *config;
...
static int kbd_simple_two_sync(int proto, int msg, int token, int param0, int param1, struct mod_virtio_smt_memory *memory)
{
    int length;
    ...
    memory->payload[0] = 0;
    length += sizeof(memory->payload[0]);
    memory->payload[1] = param0;
    length += sizeof(memory->payload[1]);
    memory->payload[2] = param1;
...
static int kbd_set_message(char *str, struct mod_virtio_smt_memory *memory)
{
    ...
    if (!strcmp(str,"version\n")) {
        ...
        return kbd_simple_two_sync(0x11, 0x4, 0xBB, i, 1, memory);
    } else    if (!strncmp(str,"power disable", 12 )) {
        sscanf(str, "power disable %d", &i);
        return kbd_simple_two_sync(0x11, 0x4, 0xBB, i, 0, memory);
...
void input_signal_message(struct mbx_device_ctx *device_ctx, unsigned int channel_idx, struct virt_msg *msg )
{
...
static gboolean input_handle_request( GIOChannel *channel, GIOCondition cond, gpointer data)
{
    ...
    if (msg.out_len > 0)
        ...
        input_signal_message(&mbx_ctx.device_ctx_table[0], 0, &msg);
    else
...
static int input_init_channels(struct mbx_device_ctx *device_ctx)
{
    GError *error = NULL;
                           ...
                           const void *data)
{
    ...
    device_ctx->config = (struct mod_input_mbx_channel_config*)data;
...
const struct fwk_module module_input_mbx = {

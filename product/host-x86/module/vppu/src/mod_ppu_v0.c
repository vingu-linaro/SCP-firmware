...
static int get_state(struct ppu_v0_reg *ppu, unsigned int *state)
{
    ...
    *state = ppu_mode_to_power_state[ppu_mode];
    if (*state == MODE_UNSUPPORTED) {
		FWK_LOG_ERR("[PD] Unexpected PPU mode (%i).\n", ppu_mode);
...
static int pd_set_state(fwk_id_t pd_id, unsigned int state)
{
    ...
    FWK_LOG_INFO("[PPU] pd_set_state pd_id %08x state %d\n", pd_id.value, state);
...
static int ppu_v0_mod_init(fwk_id_t module_id, unsigned int pd_count,
                           const void *unused)
{
	FWK_LOG_INFO("[VPPU] ppu_v0_mod_init id %04x count %u\n", module_id.value, pd_count);
...
static int ppu_v0_pd_init(fwk_id_t pd_id, unsigned int unused, const void *data)
{
    ...
    struct ppu_v0_pd_ctx *pd_ctx;
    int status;
	FWK_LOG_INFO("[VPPU] ppu_v0_pd_init id %04x\n", pd_id.value);
...
{
    struct ppu_v0_pd_ctx *pd_ctx;
    FWK_LOG_INFO("[VPPU] ppu_v0_bind id %04x round %u\n", id.value, round);
...
//        FWK_LOG_INFO("[VPPU] ppu_v0_bind bound_id %04x\n",pd_ctx->bound_id.value);
                                       ...
                                       const void **api)
{
    ...
    FWK_LOG_INFO("[VPPU] ppu_v0_process_bind_request src %04x dst %04x type %d\n", source_id.value, target_id.value, pd_ctx->config->pd_type);
...
const struct fwk_module module_vppu = {
    .name = "PPU_V0",

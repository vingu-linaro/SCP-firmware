                               ...
                               enum mod_clock_round_mode round_mode)
{
	...
	FWK_HOST_PRINT("[PLL] system_pll_set_rate id %04x rate %lu\n",dev_id.value, rate );
...
static int system_pll_get_rate(fwk_id_t dev_id, uint64_t *rate)
{
	...
	FWK_HOST_PRINT("[PLL] system_pll_get_rate id %04x\n",dev_id.value);
...
{
    struct system_pll_dev_ctx *ctx;
	FWK_HOST_PRINT("[PLLCLOCK] system_pll_set_state id %04x state %d \n",dev_id.value, state);
...
static int system_pll_get_state(fwk_id_t dev_id, enum mod_clock_state *state)
{
    ...
    *state = ctx->current_state;
	FWK_HOST_PRINT("[PLLCLOCK] system_pll_get_state id %04x state %d \n",dev_id.value, *state);
                             ...
                             const void *data)
{
	...
	FWK_HOST_PRINT("[PLL] system_pll_init id %04x count %u\n", module_id.value, element_count);
                                  ...
                                  const void *data)
{
	...
	FWK_HOST_PRINT("[PLL] system_pll_element_init id %04x \n", element_id.value);
...
{
	FWK_HOST_PRINT("[PLL] system_pll_process_bind_request src %04x dst %04x api %04x\n", requester_id.value, id.value, api_type.value);
    *api = &api_system_pll;
    return FWK_SUCCESS;
}

const struct fwk_module module_vpll = {

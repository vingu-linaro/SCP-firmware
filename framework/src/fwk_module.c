...
static int fwk_module_stop_elements(struct fwk_module_context *module_ctx)
{
    int status;
    const struct fwk_module *module;
    unsigned int element_idx;
...
static int fwk_module_stop_module(struct fwk_module_context *module_ctx)
{
    int status;
    const struct fwk_module *module;
    ...
    if (module->stop != NULL) {
        status = module->stop(module_ctx->id);
        if (!fwk_expect(status == FWK_SUCCESS)) {
            FWK_LOG_CRIT(fwk_module_err_msg_func, status, __func__);
            return status;
        }
    ...
    return fwk_module_stop_elements(module_ctx);
}
...
static int stop_modules(void)
{
    ...
    for (module_idx = 0; module_idx < FWK_MODULE_IDX_COUNT; module_idx++) {
        ...
        status = fwk_module_stop_module(module_ctx);
        if (status != FWK_SUCCESS)
            return status;
    }

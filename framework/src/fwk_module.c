...
static const char fwk_module_err_msg_line[] = "[MOD] Error %s in %s @%d";
static const char fwk_module_err_msg_func[] = "[MOD] Error %s in %s";
#endif
...
static void fwk_module_init_elements(struct fwk_module_context *ctx)
{
    ...
    for (size_t i = 0; i < ctx->element_count; i++) {
        ...
        ctx->element_ctx_table[i].state = FWK_MODULE_STATE_INITIALIZED;
    }
}
    ...
    unsigned int round)
{
    ...
    for (element_idx = 0; element_idx < fwk_mod_ctx->element_count;
        ...
        if (!fwk_expect(status == FWK_SUCCESS)) {
            FWK_LOG_CRIT(
                fwk_module_err_msg_func, fwk_status_str(status), __func__);
            return status;
        }
    ...
    unsigned int round)
{
    ...
    if (!fwk_expect(status == FWK_SUCCESS)) {
        FWK_LOG_CRIT(fwk_module_err_msg_func, fwk_status_str(status), __func__);
        return status;
    }
...
int fwk_module_start(void)
{
    ...
    if (fwk_module_ctx.initialized) {
        FWK_LOG_CRIT(
            fwk_module_err_msg_func, fwk_status_str(FWK_E_STATE), __func__);
        return FWK_E_STATE;
    }
